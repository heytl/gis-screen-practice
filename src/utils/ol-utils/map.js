...
export function fromLonLat(coordinate) {
  ...
  return Array.isArray(coordinate[0]) ? coordinate.map((n) => toWGS84(n)) : toWGS84(coordinate)
}
/**
...
export function toLonLat(coordinate) {
  if (!Array.isArray(coordinate)) return null
  if (coordinate.length === 0) return []
  return Array.isArray(coordinate[0]) ? coordinate.map((n) => toMercator(n)) : toMercator(coordinate)
}
