...
export default {
  ...
  watch: {
    type() {
      this.$emit('typeChange', this)
      if (!this.manual) {
        this.finish()
        this.draw()
        this.modify()
      } else {
        const isDraw = !!this.drawInstance
        const isModify = !!this.modifyInstance
        if (isDraw || isModify) {
          this.finish()
          if (isDraw) this.draw()
          if (isModify) this.modify()
        }
      }
    },
  },
  methods: {
    getStyle(config) {
      ...
      return typeof config === 'function' ? config() : parseStyle(config)
    },
    init(map) {
     ...
     */
    readFeature(json) {
      return new GeoJSON().readFeature(json)
    },
  },
  // created() {
  //   this.mapVm.mapReady(this.init)
  // },
  mounted() {
    this.init(this.map)
  },
