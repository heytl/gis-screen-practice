...
export default {
  ...
  methods: {
    addInteraction() {
      ...
      if (this.shape !== 'None') {
        ...
        this.draw.on('drawend', (event) => {
          const styleJSON = this.getStyleJSON(this.shape)
          if (this.shape == 'Circle') {
            // 圆形转为 Polygon 保存
            const circlePolygon = fromCircle(event.feature.getGeometry())
            event.feature.setGeometry(circlePolygon)
          }
          const style = this.getStyle(event.feature, styleJSON)
          style && event.feature.setStyle(style)
          event.feature.set('styleJSON', styleJSON)
    ...
    // 获取当前样式
    getStyle(feature, styleJSON) {
      let style;
      switch (styleJSON.shape) {
        case 'Arrow':
          style = this.arrowStyleFunction(feature, styleJSON)
          break
        default:
    ...
    getStyleJSON(shape) {
      ...
      switch (shape) {
        case 'Text':
          json = this.textEffect
          json.shape = shape
          break
        default:
          json = this.graphEffect
          json.shape = shape
          break
      }
      return json
    ...
    fromJSON(json) {
      ...
      features.forEach(f => {
        const _styleJSON = f.get('styleJSON')
        if (_styleJSON) {
          const style = this.getStyle(f, _styleJSON)
     ...
     * 根据属性渲染符号
     */
    arrowStyleFunction(feature, styleJSON) {
      let geometry = feature.getGeometry()
      let styles = []
      let lineStyle = parseStyle(styleJSON)
